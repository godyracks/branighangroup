        ...
        <div class="design-right-part"> <?php if (isset($selectedCategoryName) && $selectedCategoryName) : ?> <h4>Category Selected: <?= $selectedCategoryName ?></h4> <?php endif; ?> <div class="design-cards"> <?php foreach ($designs as $design) : ?> <div class="design-card"><a href="<?= site_url("design/show/{$design['id']}/" . url_title($design['name'], '-', TRUE)) ?>"><img src="<?= base_url('public/' . $design['image1_url']) ?>" alt="<?= $design['name'] ?>">
                        <div class="design-card-info">
                            <h4><?= $design['name'] ?></h4> <?php
                                    ...
                                    ?> <div class="design-card-footer"><span class="price"><?= $design['price'] ?></span><button class="design-buy-button">Buy</button></div>
                        </div>
                    </a></div> <?php endforeach; ?> </div>
        </div>
    </div>
    <div class="pagination"> <?php if ($currentPage > 1) : ?> <div class="page"><a href="<?= base_url('designs?page=' . ($currentPage - 1)) ?>"><i class="material-icons">arrow_back</i></a></div> <?php endif; ?> <?php for ($i = 1; $i <= $totalPages; $i++) : ?> <div class="page <?= ($i == $currentPage) ? 'active' : '' ?>"><a href="<?= base_url('designs?page=' . $i) ?>"><?= $i ?></a></div> <?php endfor; ?> <?php if ($currentPage < $totalPages) : ?> <div class="page"><a href="<?= base_url('designs?page=' . ($currentPage + 1)) ?>"><i class="material-icons">arrow_forward</i></a></div> <?php endif; ?> </div>
</main>
<script>
