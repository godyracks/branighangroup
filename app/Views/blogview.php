...
<div class="blogcontainer">
  <header>
    <input
      ...
      class="blogsearch-bar"
    />
    <div id="searchResultsContainer"></div>
  </header>
  <div class="blogcontent-wrapper">
    <div class="blogleft-column">
      <section class="latest-articles" id="latest-articles-section">
        <h2 style="color: #0CAFFF;">
          Latest Articles<i
            class="material-icons refresh-icon"
            onclick="refreshLatestArticles()"
            style="color:orange"
            >refresh</i
          >
        </h2>
        <ul id="latest-articles-list"></ul>
      </section>
     <section class="top-posts">
    <h2 style="color: #0CAFFF;">Top Posts</h2>
    <ol class="top-posts-list">
        <?php if (!empty($topPosts)): ?>
            <?php // Limiting to 5 top posts
            $count = 1; // Start numbering from 1
            foreach ($topPosts as $post): 
                if ($count > 5) break; // Limiting to 5 posts 
                ?>
                <li>
                    <div class="post-number"><?= $count++; ?></div>
                    <div class="post-content">
                        <!-- Use PHP to truncate content to 20 words -->
                        <p><?= truncate_words($post['content'], 20); ?></p>
      ...
      <section class="social-posts">
        ...
        <div class="blogsocial-icons">
          ...
          <div class="blogsocial-icon">
            <a href="https://www.instagram.com/branighan.group?igsh=Z2x3c2ZhNmxtY29s" target="_blank"><img
   ...
   <div class="article-card">
    <img src="<?= base_url('public/' . $post['article_image']) ?>" alt="<?= esc($post['title']); ?>" />
    <h3><?= esc($post['title']); ?></h3>
    <div class="article-info">
        <p class="author"><?= esc($post['author_name']); ?></p>
        <p class="published-date"><?= date('M d, Y', strtotime($post['created_at'])); ?></p>
     
        <div class="post-content"><?= truncate_words($post['content'], 15); ?></div>
    </div>
    <a href="<?= site_url('blog/view/' . urlencode(str_replace(' ', '-', $post['title']))) ?>" class="full-article-button">Full Article</a>
</div>
    ...
    <?php endforeach; ?>
      ...
      <section class="blog-pagination">
        <div class="pagination">
          ...
          1) : ?>
          <div class="page">
            <a href="<?= site_url('blog?page=' . ($currentPage - 1)) ?>"
  ...
  <section class="newsletter">
    <div class="newsletter-wrapper">
      ...
      <img
        src="<?= base_url('public/images/arcdsgnbg.jpg') ?>"
      ...
      <div class="newsletter-right">
        ...
        <div class="newsletter-info">
          <p>
            Sign up for our newsletter to receive the latest news, trends, and
            insights delivered straight to your inbox. Stay informed and never
    ...
    fetch('<?= site_url('blog/latest_articles') ?>')
        ...
        .then(data => {
            ...
            data.forEach(article => {
                ...
                let content = article.content.substring(0, 100) + '...';
                li.innerHTML = content; // Set innerHTML to preserve HTML tags
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (results.length > 0) {
        ...
        results.forEach(post => {
            ...
            var hyphenatedTitle = post.title.replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '-'); // Hyphenate without converting to lowercase

            a.href = '<?= site_url('blog/view/') ?>' + encodeURIComponent(hyphenatedTitle); // Generate hyphenated URL
...
<style>
  #searchResultsContainer ul{list-style-type:none;padding:0}#searchResultsContainer ul li{margin-bottom:5px}
</style>
<?= $this->endSection() ?>
